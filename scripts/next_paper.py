...
cat > scripts/next_paper.py << 'PYEOF'
#!/usr/bin/env python3
"""Identifica el siguiente paper del ranking que NO esta en el wiki.

Genera slug con la herramienta oficial research_wiki.py. Filtra ruido del
ranking: previews, comments, biorxiv preprints sueltos, y duplicados.
"""
...
# filename-substring -> slug real en wiki
KNOWN_ALIASES = {
...
print(f"{'='*72}")
print(f"BUSCANDO SIGUIENTE PAPER PENDIENTE (ruido filtrado)")
print(f"{'='*72}\n")

pdfs = glob.glob(f"{RAW_DIR}/*.pdf")
found_next = False
seen_slugs = set()  # para deduplicar
skipped_noise = 0
skipped_dup = 0
...
for num, title_partial in candidates:
    ...
    pdf_title = fname[:-4]
    slug = alias_hit or official_slug(pdf_title)
...
if found_next:
    ...
    print(f"{'='*72}")
    print(f"Copia esto en Claude Code:\n")
    print(f"/clear")
    print(f"/model opus")
    print(f"/effort medium")
...
chmod +x scripts/next_paper.py
.venv/bin/python scripts/next_paper.py
