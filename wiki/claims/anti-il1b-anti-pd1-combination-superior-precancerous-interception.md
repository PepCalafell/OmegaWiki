---
title: "Combined anti-IL-1β + anti-PD-1 in precancerous phase achieves the greatest reduction in tumor volume, KAC fraction, and pro-tumor inflammatory cytokines vs either monotherapy"
slug: anti-il1b-anti-pd1-combination-superior-precancerous-interception
status: supported
confidence: 0.85
tags:
  - luad
  - anti-il1b
  - anti-pd1
  - combination
  - precancer
  - mouse-model
domain: lung cancer / combination therapy
source_papers:
  - multimodal-spatial-omics-reveal-co-evolution
evidence:
  - source: multimodal-spatial-omics-reveal-co-evolution
    type: tested_by
    strength: strong
    detail: Combo arm shows greatest tumor volume reduction (especially in interception phase), greatest reduction in KAC fraction at 3 months, greatest reduction in BALF cytokines (CCL3, IL33, IL17A; plus IL23, IL21, CXCL1), highest CD8+ T cell infiltration, and counteracts anti-PD-1-associated macrophage enrichment.
  - source: mapping-inflammatory-origins-lung-cancer
    type: supports
    strength: weak
    detail: "Cancer Cell Preview: anti-IL-1β monotherapy showed stronger tumor suppression than anti-PD-1, with combination most pronounced in the interception phase; combination reduced KAC and macrophage fractions while inducing CD8+ T-cell infiltration."
conditions: "NNK Gprc5a−/− model, both prevention and interception windows."
date_proposed: 2026-05-26
date_updated: 2026-06-03
---

## Statement

Combination anti-IL-1β + anti-PD-1 administered in precancerous windows produces the most pronounced anti-tumor effects across volumetric, cellular (KAC reduction, CD8 expansion), and cytokine (CCL3/IL33/IL17A/IL23/IL21/CXCL1 reduction) endpoints, while also offsetting PD-1-induced macrophage enrichment.

## Linked ideas

- Suggests a precancer-stage interception trial: high-risk lung-cancer-screening cohorts could be candidates for IL-1β±PD-1 combination prophylaxis, contrasting with the failed CANOPY combinations in established NSCLC.
